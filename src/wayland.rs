//! Wayland renderer usando wayland-client + wayland-protocols-wlr (wlr-layer-shell)
//! Funciona en Hyprland, Sway, etc.
...
impl AppState {
    ...
    fn init_graphics(&mut self, conn: &Connection) -> Result<()> {
        ...
        let ctx_attribs = [
            ...
            khronos_egl::CONTEXT_OPENGL_PROFILE_MASK, khronos_egl::CONTEXT_OPENGL_CORE_PROFILE_BIT,
            khronos_egl::NONE,
        ];
    ...
    fn draw(&mut self) -> Result<()> {
        ...
        if let (Some(egl), Some(display), Some(surface)) = (&self.egl_instance, self.egl_display, self.egl_surface) {
...
impl WaylandRenderer {
    ...
    pub fn run(mut self) -> Result<()> {
        ...
        // Registrar los objetos globales
